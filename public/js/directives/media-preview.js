...
function mediaPreview($log,$document) {
  ...
  function _link(scope, elem, attrs, ngModel) {
    ...
    if( !attrs.previewContainer || ( !document.getElementById(attrs.previewContainer) && !angular.isElement(attrs.previewContainer) ) ) {
      ...
      // select container
      container = angular.element( elem.parent().children()[0] );
      ...
      // get the container
      container = angular.isElement(attrs.previewContainer) ?
        attrs.previewContainer : angular.element(document.getElementById(attrs.previewContainer));
    }
        ...
        angular.forEach(files, function(key, index) {
          ...
          reader.onload = function(e) {
            ...
            if( result.indexOf('data:image') > -1 ) {
              media_element = angular.element( elem.parent().children()[0].children[0] );
              // reset container
              container.empty();
              container.append( media_element );
              return container;
            }
